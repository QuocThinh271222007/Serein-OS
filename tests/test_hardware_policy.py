...
class TestMemoryPolicy:
    def test_none_configured(self, host_root):
        ...
        assert info.zram_generator_config_present is False
    ...
    def test_existing_zram_only(self, host_root):
        ...
        assert info.zram_generator_config_present is True
        assert len(info.swap_devices) == 1
...
class TestGPUPolicy:
    def test_hybrid_detected(self, host_root):
    ...
    def test_amd_desktop_discrete_only(self, host_root):
...
class TestPlanner:
    ...
    def test_nvme_scheduler_apply_when_evidence_supports_it(self, host_root):
        plan = build_hardware_plan("balanced", host_root("nvidia_workstation"))
        actions = _actions_by_id(plan)
        assert actions["storage.scheduler.nvme0n1"].status == "APPLY"
        assert actions["storage.scheduler.nvme0n1"].target == "none"
